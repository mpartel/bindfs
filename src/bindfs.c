...
#endif
{
    ...
    if (
        !dirname_access_check(real_from, W_OK)
        || !dirname_access_check(real_to, W_OK)
        || !path_has_search_perms(real_from)
        || !path_has_search_perms(real_to)
    )
        return -EACCES;
    ...
    if ((st.st_mode & S_IFDIR) == S_IFDIR && access_check(real_from, W_OK) != 0)
        return -EACCES;
